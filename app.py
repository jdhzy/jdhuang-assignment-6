...
def generate_plots(N, mu, sigma2, S):
    ...
    X = np.random.uniform(0, 1, N).reshape(-1, 1)  # Randomly generate dataset ofX of size N with values between 0 and 1
    ...
    for _ in range(S):
        ...
        Y_sim =np.random.normal(mu, np.sqrt(sigma2), N).reshape(-1, 1) # Replace with code to generate Y values
